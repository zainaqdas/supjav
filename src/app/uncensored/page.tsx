...
export default async function UncensoredPage({
  ...
  const data = await getUncensored(page, sort).catch(() => ({ videos: [] as VideoResult[], totalPages: 1, page: 1 }));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <Pagination currentPage={page} totalPages={data.totalPages} baseUrl="/uncensored" searchParams={sort ? { sort } : {}} />
    </div>
  );
}
