...
export default async function TrendingPage({
  ...
  const data = await getTrending(page, sort).catch(() => ({ videos: [] as VideoResult[], totalPages: 1, page: 1 }));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="relative mb-10">
        ...
        <div className="flex items-end justify-between gap-4">
          <SectionHeader
            title="Trending Now"
            subtitle={`Page ${data.page} of ${data.totalPages} — Most popular videos`}
          />
          <Suspense>
            <SortSelector />
          </Suspense>
      ...
      <Pagination currentPage={page} totalPages={data.totalPages} baseUrl="/trending" searchParams={sort ? { sort } : {}} />
    </div>
  );
}
